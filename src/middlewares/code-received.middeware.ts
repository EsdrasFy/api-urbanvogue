...
async function codeReceived(req: Request, res: Response, next: NextFunction) {
  ...
  if (!change || !codeFromRequest) {
    return res
      .status(401)
      .json({ msg: "The fields change and code are required." });
  }

  if (!types.includes(change as string)) {
    return res
      .status(401)
